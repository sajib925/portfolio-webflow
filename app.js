(function () {
    ...
    (function () {
      ...
      function initPlayer() {
        ...
        var video =
          ...
          document.querySelector('.scaling-video__video');
        if (!video) return;
    ...
    (function () {
      function start() {
        ...
        function build(link) {
          ...
          function make() {
            splits = [
              new SplitText(lineA, { type: 'chars' }),
              new SplitText(lineB, { type: 'chars' })
            ];
            tl = gsap.timeline({
              paused: true,
              defaults: { duration: DURATION, ease: EASE, stagger: { each: STAGGER } }
            });
            tl.to(splits[0].chars, { yPercent: -100 }, 0).to(
              splits[1].chars,
              { yPercent: -100 },
              0
            );
  ...
  // Run immediately so timing/behavior matches the original script
  // (e.g. the scroll-lock / toTop logic must run as soon as this file parses).
  initAllScripts();
})();